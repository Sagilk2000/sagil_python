...
connection= mysql.connector.connect(host="localhost",user="root",password="Sagilk@2000",database="lms")
...
#
#
def add_book(title, author, category,status):
    cursor = connection.cursor()
    query = "INSERT INTO books (title, author, category,status) VALUES (%s, %s, %s, %s)"
...
def update_book ( book_id , title , author , category ) :
    cursor = connection.cursor ( )
    query = "UPDATE books SET title = %s, author = %s, category = %s WHERE id = %s"
...
def admin_menu():
    while True:
        ...
        if choice == "1":
            ...
            category = input("Enter book Category: ")  # Corrected
            ...
            new_category = input("Enter new Category: ")  # Corrected
